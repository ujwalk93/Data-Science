setwd("D:\\PG_Diploma\\HR Analytics")
...
#business understanding and objective of case study
...
#Convert all the date columns from character to DateTime (POSIX_ct) format
...
#removing prefix X from all columns in intime_clean and outtime_clean
colnames(intime_clean) <- gsub(pattern="X","",colnames(intime_clean))
colnames(outtime_clean) <- gsub(pattern="X","",colnames(outtime_clean))

#convert data from wide to long format using gather() function
in.time_long <- gather(intime_clean, date, timein, EmployeeID)
out.time_long <- gather(outtime_clean, date, timein, EmployeeID)
...
#the derived metrics used are avg_hours, leaves, overtime and undertime

#calculating number of leaves for each employee
for(i in 1:ncol(x)){
  hrdata_mergd$leaves[i] <- sum(is.na(x[i,]))
}


#collate data into one single file hrdata_mergd
#masterData
str(hrdata_mergd)
...
#From the plots obtained, it is clear that as the work experience of employees increases the attrition rate decreases
#Also, as the years with current manager increase, attrition rate decreases.
#outliers are not removed from the numeric variables since it may affect the accuracy of final model
######################################################################@@@@@@
...
#Here we are concerned about Attrition rate, so only AttritionYes is considered
...
#JobInvolvement,JobLevel,PerformanceRating,Attrition and Gender
#Above variables have been converted into dummy variables.
#Same approach is used for both the two-level and multi-level categorical variables 
#This is because for each n-level categorical variable, n-1 dummy variables are created.
...
#Remove BusinessTravelTravel_Rarely due to high VIF (3.762446)
...
#Remove DepartmentResearch...Development due to high VIF (4.201548) 
...
#Now, remove variables based on p-value as there are quite a few variables having high p-value
#Remove PerformanceRatingOutstanding due to high p-value (0.777756)
model_9<-glm(formula = AttritionYes ~ JobRoleSales.Executive+
               JobInvolvementVery.High+EnvironmentSatisfactionVery.High+JobLevel5+
               EducationCollege+JobInvolvementLow+DepartmentSales+
...
#Remove DepartmentSales due to high p-value (0.2873)
...
#But the number of variables is still too high.
#Hence, variables are removed one by one until all of them have p-value less than 0.001
...
#Remove Age due to high p-value (0.00163)
model_19<-glm(formula = AttritionYes ~ JobRoleManufacturing.Director+WorkLifeBalanceGood+JobSatisfactionLow+WorkLifeBalanceBest+
               JobSatisfactionVery.High+NumCompaniesWorked+YearsWithCurrManager+WorkLifeBalanceBetter+BusinessTravelTravel_Frequently+
               YearsSinceLastPromotion+EnvironmentSatisfactionLow+TotalWorkingYears+MaritalStatusSingle+overtime,
             family = "binomial",data = train)
...
#Remove JobRoleManufacturing.Director due to high p-value (0.00109)
...
#now all variables have p-values less than 0.001 
#therefore, model_20 is deemed to be the final model
...
#1. If the variables have high VIF and low signifance(p-value>0.05), the variable can be removed
...
#From the final model, it is clear that following are the major factors that influence the attrition rate
#1.WorkLifeBalance
#2.JobSatisfaction
#3.NumCompaniesWorked
#4.YearsWithCurrManager
#5.BusinessTravel_Frequently
...
# Let's choose the optimum cutoff value of 0.1616 for final model
...
#each bucket indicates the respective decile
plot(x=Churn_decile$Cumlift,y=Churn_decile$bucket,type = 'o')
plot(x=Churn_decile$Gain,y=Churn_decile$bucket,type ='o')
plot(x=Churn_decile$Gain,y=Churn_decile$Cumlift,type ='o')
plot(x=max(decile),y=max(ks_table_test),type = 'o')
#From the above plots obtained, it is clear that the model predicts attrition more accurately as most of the values are accurately predicted by the 4th decile.
#From the optimum probability cut-off (0.1616) and the ROC curve obtained, it is clear that the model is the best fit when it comes to accuracy.
#From the gain and lift charts obtained, it is clear that the model has increasing gain and decreasing lift.
#It is clear that the model predicts attrition more accurately as most of the values are accurately predicted by the 4th decile.